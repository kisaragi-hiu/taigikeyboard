//! Brand and mail glyphs for the settings footer's community links, as
//! XAML path data for a `PathIcon`. Port of `FontAwesomeGlyph.swift`.
//!
//! Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com
//! License - https://fontawesome.com/license/free (Icons: CC BY 4.0)
//! Copyright 2024 Fonticons, Inc.
//!
//! Path data from `svgs/regular/heart.svg`, `svgs/solid/globe.svg`,
//! `svgs/brands/github.svg`, `svgs/brands/discord.svg` and
//! `svgs/regular/envelope.svg`, rewritten as absolute commands and scaled to
//! a 16-unit height: `PathIcon` draws its geometry at its own size, and the
//! same string is what macOS decodes. Segoe Fluent Icons carries no brand
//! marks.

use windows_reactor::*;

/// A Font Awesome glyph, 16 units tall.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum FontAwesomeGlyph {
    Heart,
    Globe,
    Github,
    Discord,
    Envelope,
}

impl FontAwesomeGlyph {
    /// The glyph as an icon; it takes the foreground of the control around it.
    ///
    /// `F1` asks for the nonzero fill rule SVG defaults to, where XAML
    /// defaults to even-odd: the same string, the same holes.
    pub fn icon(self) -> PathIcon {
        PathIcon::new().data(format!("F1 {}", self.path_data()))
    }

    fn path_data(self) -> &'static str {
        match self {
            Self::Heart => HEART,
            Self::Globe => GLOBE,
            Self::Github => GITHUB,
            Self::Discord => DISCORD,
            Self::Envelope => ENVELOPE,
        }
    }
}

const HEART: &str = "M 7.05,14.63 L 6.97,14.55 L 1.5,9.47 C 0.54,8.58 0,7.33 0,6.02 L 0,5.92 C 0,3.72 1.56,1.83 3.72,1.42 C 4.95,1.18 6.21,1.46 7.21,2.17 C 7.5,2.37 7.76,2.6 8,2.87 C 8.13,2.72 8.27,2.58 8.42,2.45 C 8.53,2.35 8.65,2.26 8.78,2.17 C 8.78,2.17 8.78,2.17 8.78,2.17 C 9.78,1.46 11.04,1.18 12.27,1.41 C 14.43,1.83 15.99,3.72 16,5.92 L 16,6.02 C 16,7.33 15.45,8.58 14.49,9.47 L 9.02,14.55 L 8.94,14.63 C 8.68,14.86 8.35,15 8,15 C 7.65,15 7.31,14.87 7.05,14.63 M 7.47,4.53 C 7.45,4.52 7.45,4.5 7.44,4.49 L 6.88,3.87 L 6.88,3.86 C 6.88,3.86 6.88,3.86 6.88,3.86 C 6.15,3.05 5.06,2.69 4,2.89 C 2.55,3.17 1.5,4.44 1.5,5.92 L 1.5,6.02 C 1.5,6.91 1.87,7.76 2.52,8.37 L 8,13.45 L 13.47,8.37 C 14.12,7.76 14.5,6.91 14.5,6.02 L 14.5,5.92 C 14.5,4.44 13.45,3.17 11.99,2.89 C 10.93,2.69 9.84,3.06 9.12,3.86 C 9.12,3.86 9.12,3.86 9.11,3.87 C 9.11,3.87 9.11,3.87 9.11,3.87 L 8.55,4.5 C 8.54,4.51 8.53,4.52 8.52,4.53 C 8.38,4.67 8.19,4.75 7.99,4.75 C 7.8,4.75 7.61,4.67 7.47,4.53 L 7.47,4.53";

const GLOBE: &str = "M 11,8 C 11,8.69 10.96,9.36 10.89,10 L 5.1,10 C 5.03,9.36 5,8.69 5,8 C 5,7.3 5.04,6.63 5.1,6 L 10.89,6 C 10.96,6.63 11,7.3 11,7.99 M 11.9,6 L 15.74,6 C 15.91,6.64 16,7.3 16,8 C 16,8.69 15.91,9.35 15.74,10 L 11.89,10 C 11.96,9.35 11.99,8.68 11.99,8 C 11.99,7.31 11.96,6.64 11.89,6 L 11.9,6 M 15.41,5 L 11.77,5 C 11.45,3 10.84,1.33 10.04,0.26 C 12.49,0.9 14.48,2.68 15.41,5 L 15.41,5 M 10.75,5 L 5.24,5 C 5.43,3.86 5.72,2.85 6.08,2.04 C 6.41,1.3 6.77,0.76 7.13,0.43 C 7.48,0.09 7.77,-5.55e-17 8,0 C 8.22,0 8.51,0.1 8.86,0.43 C 9.22,0.76 9.58,1.3 9.91,2.04 C 10.27,2.85 10.56,3.85 10.75,5 L 10.75,5 M 4.22,5 L 0.58,5 C 1.51,2.68 3.5,0.9 5.95,0.26 C 5.15,1.33 4.54,3 4.22,5 L 4.22,5 M 0.25,6 L 4.1,6 C 4.03,6.64 3.99,7.31 3.99,8 C 3.99,8.68 4.03,9.35 4.1,10 L 0.25,10 C 0.08,9.35 0,8.69 0,8 C 0,7.3 0.08,6.64 0.25,6 M 6.08,13.95 C 5.72,13.14 5.43,12.13 5.24,11 L 10.75,11 C 10.56,12.13 10.27,13.14 9.91,13.95 C 9.58,14.69 9.22,15.22 8.86,15.56 C 8.51,15.9 8.22,16 8,16 C 7.77,16 7.48,15.9 7.13,15.56 C 6.77,15.23 6.41,14.69 6.08,13.95 L 6.08,13.95 M 4.22,11 C 4.54,12.99 5.15,14.66 5.95,15.73 C 3.5,15.09 1.51,13.31 0.58,11 L 4.22,11 M 15.41,11 C 14.48,13.31 12.49,15.09 10.04,15.73 C 10.84,14.66 11.45,12.99 11.77,11 L 15.42,11 L 15.41,11";

const GITHUB: &str = "M 5.18,12.41 C 5.18,12.48 5.11,12.53 5.02,12.53 C 4.91,12.54 4.84,12.49 4.84,12.41 C 4.84,12.35 4.91,12.3 5,12.3 C 5.1,12.29 5.18,12.34 5.18,12.41 M 4.21,12.27 C 4.19,12.34 4.25,12.41 4.34,12.43 C 4.42,12.46 4.52,12.43 4.54,12.36 C 4.55,12.3 4.5,12.23 4.4,12.2 C 4.32,12.18 4.23,12.21 4.21,12.27 M 5.59,12.22 C 5.5,12.24 5.44,12.3 5.45,12.37 C 5.45,12.44 5.54,12.48 5.63,12.45 C 5.72,12.43 5.78,12.37 5.77,12.31 C 5.76,12.25 5.68,12.21 5.59,12.22 M 7.65,0.25 C 3.31,0.25 8.88e-16,3.54 0,7.87 C 0,11.34 2.18,14.3 5.29,15.35 C 5.69,15.42 5.83,15.17 5.83,14.97 C 5.83,14.77 5.82,13.7 5.82,13.05 C 5.82,13.05 3.64,13.52 3.18,12.12 C 3.18,12.12 2.82,11.21 2.31,10.97 C 2.31,10.97 1.59,10.48 2.36,10.49 C 2.36,10.49 3.14,10.55 3.56,11.3 C 4.25,12.5 5.39,12.16 5.84,11.95 C 5.91,11.45 6.12,11.1 6.34,10.9 C 4.6,10.7 2.83,10.45 2.83,7.44 C 2.83,6.59 3.07,6.15 3.57,5.6 C 3.49,5.4 3.22,4.56 3.65,3.48 C 4.3,3.28 5.81,4.33 5.81,4.33 C 6.43,4.15 7.1,4.06 7.77,4.06 C 8.44,4.06 9.11,4.15 9.73,4.33 C 9.73,4.33 11.24,3.28 11.89,3.48 C 12.32,4.57 12.05,5.4 11.97,5.6 C 12.47,6.16 12.78,6.59 12.78,7.44 C 12.78,10.46 10.94,10.7 9.19,10.9 C 9.48,11.14 9.72,11.61 9.72,12.35 C 9.72,13.4 9.71,14.7 9.71,14.96 C 9.71,15.16 9.85,15.41 10.25,15.34 C 13.38,14.3 15.5,11.34 15.5,7.87 C 15.5,3.54 11.98,0.25 7.65,0.25 M 3.03,11.02 C 2.99,11.05 3,11.13 3.05,11.19 C 3.1,11.24 3.18,11.26 3.22,11.22 C 3.26,11.19 3.25,11.11 3.2,11.05 C 3.15,11 3.07,10.98 3.03,11.02 M 2.7,10.77 C 2.67,10.81 2.7,10.86 2.77,10.89 C 2.82,10.92 2.88,10.91 2.9,10.87 C 2.92,10.83 2.89,10.78 2.83,10.75 C 2.77,10.73 2.72,10.74 2.7,10.77 M 3.71,11.88 C 3.66,11.92 3.68,12.02 3.75,12.08 C 3.82,12.15 3.91,12.16 3.95,12.11 C 3.99,12.07 3.97,11.97 3.91,11.91 C 3.84,11.84 3.75,11.83 3.71,11.88 M 3.35,11.42 C 3.3,11.45 3.3,11.54 3.35,11.61 C 3.4,11.68 3.49,11.71 3.53,11.68 C 3.58,11.64 3.58,11.56 3.53,11.49 C 3.48,11.41 3.4,11.38 3.35,11.42";

const DISCORD: &str = "M 16.39,2.18 A 0.04,0.04 0 0 0 16.36,2.16 A 15.15,15.15 0 0 0 12.62,1 A 0.05,0.05 0 0 0 12.56,1.02 A 10.54,10.54 0 0 0 12.1,1.98 A 13.99,13.99 0 0 0 7.9,1.98 A 9.67,9.67 0 0 0 7.42,1.02 A 0.05,0.05 0 0 0 7.36,1 A 15.11,15.11 0 0 0 3.62,2.16 A 0.05,0.05 0 0 0 3.6,2.18 C 1.22,5.73 0.56,9.2 0.88,12.63 A 0.06,0.06 0 0 0 0.91,12.67 A 15.23,15.23 0 0 0 5.5,14.99 A 0.05,0.05 0 0 0 5.56,14.97 A 10.88,10.88 0 0 0 6.5,13.45 A 0.05,0.05 0 0 0 6.47,13.36 A 10.03,10.03 0 0 1 5.03,12.68 A 0.05,0.05 0 0 1 5.03,12.58 C 5.12,12.51 5.22,12.44 5.31,12.36 A 0.05,0.05 0 0 1 5.37,12.35 C 8.38,13.72 11.63,13.72 14.61,12.35 A 0.05,0.05 0 0 1 14.67,12.36 C 14.76,12.44 14.85,12.51 14.95,12.58 A 0.05,0.05 0 0 1 14.95,12.68 A 9.41,9.41 0 0 1 13.51,13.36 A 0.05,0.05 0 0 0 13.48,13.45 A 12.22,12.22 0 0 0 14.42,14.97 A 0.05,0.05 0 0 0 14.48,14.99 A 15.18,15.18 0 0 0 19.08,12.67 A 0.05,0.05 0 0 0 19.1,12.63 C 19.49,8.67 18.46,5.23 16.39,2.18 M 6.95,10.54 C 6.04,10.54 5.3,9.71 5.3,8.69 C 5.3,7.67 6.03,6.84 6.95,6.84 C 7.87,6.84 8.61,7.68 8.6,8.69 C 8.6,9.71 7.87,10.54 6.95,10.54 M 13.05,10.54 C 12.15,10.54 11.4,9.71 11.4,8.69 C 11.4,7.67 12.13,6.84 13.05,6.84 C 13.98,6.84 14.72,7.68 14.7,8.69 C 14.7,9.71 13.98,10.54 13.05,10.54";

const ENVELOPE: &str = "M 2,3.5 C 1.72,3.5 1.5,3.72 1.5,4 L 1.5,4.69 L 6.89,9.11 C 7.53,9.64 8.46,9.64 9.11,9.11 L 14.5,4.69 L 14.5,4 C 14.5,3.72 14.27,3.5 14,3.5 L 2,3.5 M 1.5,6.63 L 1.5,12 C 1.5,12.27 1.72,12.5 2,12.5 L 14,12.5 C 14.27,12.5 14.5,12.27 14.5,12 L 14.5,6.63 L 10.06,10.27 C 8.86,11.25 7.13,11.25 5.93,10.27 L 1.5,6.63 M 0,4 C 0,2.89 0.89,2 2,2 L 14,2 C 15.1,2 16,2.89 16,4 L 16,12 C 16,13.1 15.1,14 14,14 L 2,14 C 0.89,14 0,13.1 0,12 L 0,4";
