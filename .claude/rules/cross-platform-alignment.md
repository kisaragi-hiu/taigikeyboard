...
Governs every PR that touches a file marked `Shared-Core Candidate` (the residual `native_pending` / `native_keep` roster in `docs/engine/migration-inventory.csv`). Each bug fix must **shrink** future Rust migration cost by keeping the candidate surface clean, not accumulate platform-specific scar tissue.
...
- Constants that mirror the other platform carry a `CROSS-PLATFORM INVARIANT — mirrors <other file>:<line>. Drift causes silent divergence.` comment. Current examples (`grep -rl "CROSS-PLATFORM INVARIANT"`): iOS `Settings/EngineSettings.swift` / `Settings/BuiltInThemes.swift` / `Input/AutoSpacePunctuation.swift` / `Lexicon/Utils/LexiconBitmask.swift` / `Autocomplete/Services/SuggestionCaseTransformer.swift`; Android `ime/core/settings/EngineSettings.kt` / `ime/core/BuiltInThemes.kt` / `ime/text/AutoSpacePunctuation.kt` / `ime/dictionary/LexiconBitmask.kt` / `ime/dictionary/SuggestionCaseTransformer.kt`. Files marked `// MARK: - Shared-Core Candidate` (iOS, roster in `ios-shared-core-candidates.md`) are platform-stays helpers whose canonical logic already lives in the Rust engine; because Android JVM unit tests cannot load the `.so`, their invariant is pinned iOS↔Android in source rather than via FFI.
