...
- **L10 Update check: none in-app (unsupported host capability, deliberately).** Linux
  packages are updated by the package manager, and no mainstream IBus engine checks for
  updates itself. The 一般 pane shows the running version and a 下載頁 link
  (`gtk::UriLauncher`); the `update*` settings keys stay reserved and unwritten;
  `taigi-windows-update` is not linked. The appcast / `_data/*_release.json` machinery is
  untouched — the announce workflow gains a Linux asset only when § L11's package is
  staged on the draft.
...
| 教典 off: its eleven 腔口 rows | shown, greyed | shown, greyed | collapsed under the `adw::ExpanderRow` (its enable switch IS the toggle), choices kept | platform-adapted presentation |
| Sidebar icons | yes | yes | none | named divergence |
...
| PR7 | Settings II | 快捷鍵 (recorder over `EventControllerKey`, both registries, conflicts, slot-key-set picker), 詞庫來源 (教典 subcollections in an `adw::ExpanderRow`) | branch `feat/linux-settings-shortcuts` (stacked on PR6 locally; PR opens against main once #148 merges) — 快速齒: three `PreferencesGroup`s (選字 / 輸出 / 其他), recorder rows = `ActionRow` + field button + × (`recorder.rs`: `RecorderTarget` port, `Recorder` with a held-key repeat latch), keys captured by one `gtk::EventControllerKey` on the window in the capture phase while a row records (the same `taigi-linux-platform::snapshot` translation the engine applies, so a recorded chord is the chord the key path matches), focus loss ends a recording; fixed rows as subtitles (slot keys follow 聲調拍法 on refresh); reset = both registries. 詞庫來源: 教典 as an `adw::ExpanderRow` whose enable switch is `IS_KAUTIAN_ENABLED` with the eleven 腔口 rows inside; MOE others / 其他 / 增補 groups; reset. Pane test: recorder binds / refuses / clears / resolves across registries, pane switch and any other write end a recording, expander round-trip. Codex post-impl **FIX → applied**: a recording ends on a pane switch, any other row's write, a rebuild, or focus leaving the field (`focus-widget` notify); the held-key set is by **keycode** (a repeat whose keysym changed under a joining modifier is still a repeat) and a consumed key stays swallowed until its release even after the recording ended; 教典-off collapse recorded as a named divergence |
...
| Distro-managed updates, no in-app updater | `ibus-rime`, `ibus-mozc`, `ibus-chewing` packaging | L10 |
