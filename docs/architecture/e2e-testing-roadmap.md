...
| PR6 | Windows: TSF events, `/IT` interactive-session `SendInput` driver over `ssh win` | Dropped — USER 2026-09-24 「skip windows」, after PR6a's first box build hit `os error 4551` (Windows application control blocked a build binary). Unmerged work on branch `feat/e2e-windows-trace` (`04b2fe0b`): shared `taigi-desktop-core::trace`, TSF `e2e-trace` feature, second GUID set, `make build E2E=1` |
