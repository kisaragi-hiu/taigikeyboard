...
| PR4 | Linux: CI matrix Ubuntu 24.04 / Debian 13 / Fedora 44 / Arch × {Fcitx5, IBus}, each container building `make install E2E=1` from source against its own libraries (packages are never traced — PR3a; the shipped packages keep their `install-check` job). `linux-e2e.yml` becomes one container matrix (`linux-e2e-<distro>` artifacts) | In progress (branch `feat/e2e-linux-distro-matrix`) |
