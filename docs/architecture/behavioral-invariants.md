...
- **A candidate with no Hanji writes its romanization under every mode.** The §34 字面羅馬字 candidate (at slot 0 whenever 顯示當咧拍的字 is on, which is the shipped default on all four platforms), an out-of-vocabulary name, a romanization-only custom entry. The old proxy called all of them hanji commits in 漢字優先 and in 漢羅濫 (which forces the swap on), so `taigi` + Return landed unspaced while the same list's dictionary romanization cell was spaced.
- **The 漢羅 key / a §42 split cell commits the script the mode does NOT lead with.** Already handled on the desktop (`.alternate`) and for marked mobile cells before this round; the verdict now carries it uniformly.
...
| Hanji candidate, 括號標註 ON | `漢字 (羅馬字)` / `羅馬字 (漢字)` | true |
| Hanji candidate, 漢字-led, brackets OFF | the 漢字 | **false** |
| Hanji candidate, roman-led, brackets OFF | the romanization | true |
| the other script (Space / 漢羅 key / §42 roman cell) | that script alone | that script |
...
**The swap asks about PROVENANCE, not about the mode.** The arm is set only where a space is actually written, so its existence IS the verdict — the desktop stores the caret position it must re-verify (`armedAutoSpaceCaret` / `ContextState::armed_auto_space`), mobile a bool (`isAutoSpaceArmed`). Only `isAutoSpaceEnabled` is re-read live at swap time: what is in front of the caret is a fact about a commit that already happened, so flipping 漢字優先 between the commit and the punctuation key no longer turns an auto space into a stale one (it did on the desktop before this round).
...
- **Gating** — the swap fires only inside the same gate the auto-space insertion sites use, so it never eats a user-typed space when auto-space is off, after a 漢字 commit, or after a TPS Bopomofo commit. The document (cursor-preceding char) still has to agree — mobile checks it, the desktop verifies the caret range as well — but the remembered verdict is what says the space was ours to begin with. `false` until the first commit, so a space the user typed themselves is never swapped.
- **Known adjacent approximation (not fixed here)** — 全形標點 still reads the output MODE, so a punctuation key typed mid-composition in 漢字優先 gets the full-width glyph while the raw preedit it commits earns the roman space: `taigi？ `. Which marks 漢字 mode types is a 全形標點 policy question, deliberately left to its own round.
...
One seam answers both the drawn labels and the key handler — `CandidateIndexLabel` on both platforms (`candidateIndex(forKeySlot:leadCellIsUnkeyed:indexForSlot:)` / `candidate_index_for_key_slot`, with `keySlotShift` / `key_slot_shift` hoisted for a repaint) — so a key drawn beside a cell is the key that commits it. Whether the lead cell is the literal is decided platform-side (macOS `ComposingManager.leadsWithLiteralRomanCandidate(_:settings:)`, Windows `composing::presentation::leads_with_literal_roman`; both answered from the same settings snapshot the cells are presented under): the setting is on AND the leading candidate carries no hanji, which is what the engine's prepend produces; the desktop has no TPS mode, the engine's other gate.
 ⇧Return (`commitLiteral`, raw `CommitRaw`) stays as the raw escape hatch that ignores the highlight. Space on the literal cell (one script) still answers `.ignored` (§42). Under 羅馬字 (§44) the literal collapses a same-roman dictionary row into itself and keeps slot 0; under 漢羅濫 (S27) the literal cell leads and the first Hanji cell follows. macOS additionally: the raw name `literalRomanCandidateEnabled` was tombstoned in `RetiredSettingsCleanup` while #669 had no row for it — **the tombstone is gone**, or the launch sweep would clear the setting the 一般 pane writes (which is exactly how the #558-era row had been left dead). The retired `toggleLiteralRomanCandidate` shortcut stays retired: this round restores a settings row, not a hotkey. Windows keeps no retired-key sweep, so its `settings.json` value is simply read. ⚠ Pre-existing gap (surfaced 2026-09-02, not fixed here): `AutoSpacePolicy` (macOS `Controller/AutoSpacePolicy.swift`, Windows `policies/auto_space.rs`) decides the space from `CandidateScript` + the effective swap, and a one-script literal is presented as `.primary`, so Return on the literal under 漢字-first side-by-side or 漢羅濫 may NOT earn the auto-space a romanization commit should — the roman-first and 羅馬字 modes do.
...
**Engine sites**: `phonetics/src/tps.rs` (`tps_notone_prefix_boundary_tone`, plus the extracted `tl_syllable_tokens` / `tps_notone_from_numeric_token` the three TPS derivations now share), `composing/src/derived.rs` (`strip_tps_separator_markers`, used by `derived_display` and the commit / NextWord seams), `composing/src/transition.rs` (bare-`Composing` `CommitRaw`, terminal NextWord roman), `composing/src/shadow.rs` (`span_end_pins_unmarked_tone`, `span_tone_pin` / `whole_buffer_tone_pin`, `ContinuousKeys::tone_pins`, trailing-separator endpoint correction in `lattice_from_canonical_with_barriers`), `composing/src/continuous.rs` (per-key + per-edge plumbing, ctx body), `lexicon/src/continuous.rs` (`is_unmarked_tps_tone`, `passes_space_pinned_tone`, and the guard in `fetch_candidates_for_keys_with_barriers` / `best_candidate_for_key_with_barriers` / `fetch_partial_prefix_candidates_unbounded` / both custom merges).
...
Every candidate cell shows the 漢字 AND the 羅馬字 of the word it offers, on all four platforms, under the default 候選詞顯示 = 漢羅對應. The swap setting decides which one LEADS; it never decides whether the other appears.
...
**A second USER-requested exception (2026-08-30 「不需要分成 title/subtitle，漢字和羅馬字會一併出現」; corrected 2026-09-02 「漢字單獨做一個候選詞、羅馬字也當作候選詞,也沒有 subtitle」): 候選詞顯示 = 漢羅濫.** Both scripts still appear, but **as two adjacent one-script candidates, not as one cell**: for each `(漢字, 羅馬字)` the window lists a 漢字 cell and then a 羅馬字 cell, neither with a subtitle / annotation; a hanji-less candidate (§34 literal, roman-only custom entry) lists its roman cell alone. Picking a cell commits that cell's script (Enter / slot key); Space commits the other script of the same candidate. **Cells that read the same are listed once, per script, first-seen wins** — 食/𤆬 share one `tsia̍h`; the literal `tâi` absorbs 台's roman cell; 重/tîng and 重/tāng draw ONE 重 cell and keep both roman cells, so the losing reading stays reachable through its own romanization. A one-script cell shows nothing that could tell it from an earlier cell reading the same (the span is not on screen either), so a second one is a defect, not a second offer — USER 2026-09-03 「相同的漢字 or 羅馬字不能重複出現」. The two scripts keep separate keys, and 漢羅對應 is untouched: its subtitle tells 重/tîng from 重/tāng. Hanji cells were exempt until 2026-09-03 on Core Principle #7 grounds (they ARE two words — but the identity survives on the roman cell, not on a second identical 漢字 cell). Both cells commit the SAME candidate — identity sidechannels unchanged, only the document script differs, exactly as Space does under 並排. The platforms express the pair's lead by projecting the mode onto the existing swap pair — effective `isTranslateSwapped = true` (`.primary` = hanji, `.alternate` = roman), `isOutputBothScripts = stored` — a **compatibility projection**, not a claim that 合用 is hanji-first in any deeper sense: auto-space follows the script actually committed (a roman cell spaces, a hanji cell does not), 選 label / the nextword Enter-raw gate behave as today's hanji-first mode, 括號標註 applies to a hanji-cell commit as `漢字 (羅馬字)`. **Full-width punctuation is NOT part of the projection (2026-09-13, USER 「漢羅濫需要有 isTranslateSwapped 的按鈕，因為仍然有需要全形、半形標點符號的切換」)**. The rule, four platforms: 羅馬字 masks every stored script flag (`showsHanji`); 濫 forces only the candidate lead (`effectiveTranslateSwapped`); everything else — 括號標註, punctuation width (`effectiveFullWidthPunctuation`), the 文/A key / swap shortcut (`allowsSwapToggle`) — follows the stored value. So the key stays on mobile (bottom row + expanded overlay) and the desktop `` ` `` shortcut stays live under 濫 — both flip the stored swap, which there changes only the punctuation width; under 羅馬字 the width is always half (key hidden, shortcut inert). **TPS is always full-width on every page** (character / numeric / symbolic — the platform derived getter ORs `isTpsLayout` in, same seam on iOS and Android), and the 文/A key is hidden there (bottom row + overlay; toggle guarded) since it could flip nothing — 2026-09-13 parity fix: Android's symbol pages used to follow the stored swap under TPS while iOS forced full-width. No engine reader: neither §44 pass runs (the split happens in the platform presentation layer). **Desktop shipped the split first (#666); iOS/Android follow in #668** with the platform-shaped differences only: a tap commits that cell's script (mobile has no Space-commits-the-other path), the strip's height is the smartbar's (platform-fixed) rather than a floating window's. **NextWord prediction rows follow the picker too (2026-09-12, USER 「在漢羅濫,必須同時出現羅馬字和漢字建議,如果為「羅馬字」模式，則必須只出現羅馬字,如果是「漢羅對應」,則出現原本的「漢羅對應」」; #668 had left them hanji-led under 濫)**: under 濫 a prediction with romanization is a 漢字 cell then a 羅馬字 cell (`NextWordHandler.buildPredictionWords` / `ActionHandler.predictionSuggestions`), same marker, same per-script first-seen dedupe, both cells sharing the prediction's identity; a hanji-only prediction lists its 漢字 cell alone; the marked cell commits its own script through the same resolver the Continuous split uses. Under 羅馬字 the roman alone, under 並排 today's dual-script cell. Research: the same report, Part I §1-§11.

**The §34 literal inherits the identity of the row it absorbs (2026-09-12).** Under a single-script display (羅馬字 via `dedupe_display_roman`, 濫 via the platform roman-cell dedupe) the dictionary row that reads like the literal is no longer a cell of its own, so tapping the literal is the only way to commit that word — and the commit used to carry the literal's roman as its identity (`display_text = roman`), which the Hanji-keyed NextWord lookup (`NextWordService.predict` / `NextWordRepository.fetchUserRows`) never matched: no prediction ever followed, and 詞頻 learned a `(roman, tl)` bucket. `composing::dispatch::adopt_collapsed_dict_identity` now copies the absorbed row's `display_text` + `canonical_tl` onto the literal while `roman` / `hanji: None` stay — the cell still reads, orders and writes the exact preedit literal; first-seen (top-ranked) wins among 同音異字, the same rule that picks the visible cell. Deliberate consequence: a single-syllable literal that inherits a dictionary word joins the `api::nailed_prefix` compound run (`台` + literal `gí` → `tâi-gí`), exactly as the absorbed dictionary cell would have. 並排 is untouched: its dictionary row keeps its own cell and the literal stays a pure romanization commit. This is a documented exception to "identity is mode-independent" (decision b): the same literal tap learns `(隻, tsiah)` under 羅馬字 / 濫 and `(tsiah, tsiah)` under 並排, because only under 並排 could the user have picked 隻's own cell instead.
...
- **History.** The first cut (PR #82 commit `aa541d28`) kept the abbreviation keys in the phonetic families and compensated with a consonant-only shape gate plus a per-row face check, which made vowel-initial abbreviations unreachable; the family tag replaced both the same day (USER 2026-09-18: 「無聲母个詞…凡勢愛掠頭一個字母出來鬥縮寫」). Still visible: the custom-dictionary abbrev match is `LIKE ? || '%'` (prefix) on all four platforms while this dictionary lookup is exact; pre-existing platform SQL, not changed here.
...
**Reported by**: Facebook video relayed 2026-09-18 — 「開符號選單的時陣，揤↑↓←→了後拍字的位會家己走去，而且揤「Enter」拍袂出來」.

## §48 — Hanji leads out of the box; the desktop 一般 pane owns the output script and its own reset

### `INVARIANT_HANJI_FIRST_DEFAULT`

Until 2026-09-18 every platform shipped `isTranslateSwapped = false`: a fresh install led every 並排 cell with the romanization, wrote the romanization on commit, and typed half-width punctuation, while the 文/A key (mobile) and the `` ` `` chord (desktop) were the only way to the hanji lead. USER 2026-09-18: 「確認預設輸出都是漢字，不是羅馬字」, then, once the audit showed mobile shipped roman-first too, 「一起修復，預設都是先輸出漢字，也就是漢字是 title，羅馬字是 subtitle」.

- **The stored swap defaults to `true` on all four platforms.** macOS `EngineSettings.defaults` and Windows `EngineSettings::DEFAULT` (one `const`, which `keys.rs` and `Default` both read — the former `engine_defaults()` copy and its pin test are gone) each name the stored swap once and DERIVE the snapshot's effective `isTranslateSwapped` / `isFullWidthPunctuation` from it under 並排, so a defaults snapshot cannot disagree with itself; iOS `SharedSettings.isTranslateSwappedKey` default + `resetToDefaults`; Android `PrefHelper.storedIsTranslateSwapped` default + reset. The engine-side `AppConfig` struct defaults are wire defaults every platform overrides; they did not move.
- **A never-stored key moves with the default; a stored one does not.** The readers all follow "absent → default" (`SettingsStore.bool`, `SettingsDocument::bool`, `SettingsKey.bool(default:)`, `by preference(…, true)`), so an install that never pressed the swap key becomes hanji-first on update, and one that did keeps the value it wrote — including a `false` that means "I chose 羅馬字". No migration writes the default through, for the reason every desktop reset states: a written default is indistinguishable from a choice.
- **Derivation is untouched.** §42's `effectiveTranslateSwapped` / `effectiveOutputBothScripts` / `effectiveFullWidthPunctuation` still read the stored value through the display mode: 羅馬字 masks it, 漢羅濫 forces the lead and leaves the width to it.
- **The mobile 文/A key lights for the NON-default (USER 2026-09-18 「ok fix it for mobile」).** iOS `ButtonImageProvider` draws `character.square.fill` and Android `KeyContent.kt` paints `keyBgActive` when `isFullWidthPunctuation` is FALSE — 羅馬字 lead, half-width marks; the shipped state (hanji, full-width) is the plain outlined / unpainted key, so a fresh install shows no key "on" that the user never switched on. Presentation only: nothing reads the lit state back (layouts, commit script and candidate lead each read the derived flags). The Android theme preview (`KeyboardPreviewPanel.kt`) renders the shipped state (`isFullWidthPunctuation = true`, 選 label).

### `INVARIANT_DESKTOP_GENERAL_PANE_OUTPUT_SCRIPT_AND_RESET`

- **輸出文字 is a row on the 一般 pane, bound to the stored swap.** Labelled `settings.outputScript` (輸出文字), paired with `settings.inputScript` (輸入文字) on the TL / POJ picker (USER 2026-09-18 「輸入/輸出文字好像比較好」; mobile keeps `inputMode` 輸入模式 — its picker also holds TPS). The pane is one run of rows, no sub-groups (USER 2026-09-18 「不要分組」), in typing-pipeline order since 2026-09-21 (USER, superseding the 2026-09-18 「輸出輸入可以排在一起」 pairing): 輸入文字 · 聲調拍法 · 顯示選字窗 · 當咧拍的字囥第一个 · 輸出文字 · 無連字符 · 自動閬一个縫 · 介面語言, then the version row. macOS `GeneralSettingsView` `@AppStorage(Keys.isTranslateSwapped)` picker, `漢字` = `true`, `羅馬字` = `false`; Windows `general.rs` `choice_row` over `OUTPUT_SCRIPTS = [true, false]` writing `Message::SetSwitch(IS_TRANSLATE_SWAPPED, …)`. Same key the `` ` `` chord flips (`TaigiInputController.performShortcutAction .toggleTranslateSwapped`, `session.rs ShortcutAction::ToggleTranslateSwapped`), so the chord and the pop-up are one setting seen from two places — a chord press repaints the open pane through the binding / the live-reload beat.
- **Disabled exactly where the chord is inert.** `CandidateDisplayMode.allowsSwapToggle` / `allows_swap_toggle` (§42): greyed under 羅馬字 with the stored value still showing; enabled under 漢羅濫, where — like the chord — it only decides punctuation width. `cards::choice_row` grew the same `is_enabled` parameter `switch_row` has for this.
- **恢復預設設定 at the foot of the pane, same shape as 外觀 / 快捷鍵 / 辭典管理.** macOS `WideActionRow(.themeEditorResetAll)` → `SettingsStore.resetGeneralSettings()`; Windows `pages::reset_row` (the one gap-plus-card shape all four panes now share) → `ResetScope::General` → `SettingsDocument::reset_general()` over `keys::GENERAL_KEYS`. The roster is spelled out — `inputMode`, `toneInputScheme`, `isTranslateSwapped`, `autoSpaceEnabled`, `candidateWindowEnabled`, `literalRomanCandidateEnabled`, `hyphenlessRomanEnabled` — and removed, not written. Not in it: `displayLanguage` (USER 2026-09-20: the reset must not switch the UI language under the user), the update bookkeeping and the remembered pane (not the user's settings), and every other pane's keys.
...
## §49 — 無連字符 strips the romanization the user sees, never the keys the engine learns by

### `INVARIANT_HYPHENLESS_ROMAN_DISPLAY_ONLY`

USER 2026-09-20: 「評估增加一個無連字符開關,當開關開啟,候選詞不需要有連字符，例如：tâi-uân -> tâiuân … 針對輕聲記號"--",則改用一點「·」取代」, then 「只去掉引擎/詞庫產生的」 (user-typed hyphens stay), 「確保不要影響到TPS和其他功能」, 「使用者自訂詞庫也必須跟著這個設定」.
...
- **One key spelling, host settings only.** `hyphenlessRomanEnabled` on iOS (`SharedSettings`), macOS (`SettingsStore.Keys`, 一般 pane under 輸出文字, in `resetGeneralSettings`) and Windows (`keys::IS_HYPHENLESS_ROMAN_ENABLED`, `GENERAL_KEYS`, 一般 page under 輸出文字); Android `keyboard__hyphenless_roman`. Mobile: the host-app 輸入 section under 括號標註 — no in-keyboard overlay row (USER 2026-09-20). Default OFF everywhere.
...
- **Result**: 台日 off (default) + learned 記起來 → 記起來 is the first hanji candidate for `kikhilai` after ONE composition; 台日 on → the dictionary row leads and the pair is listed once; a mistaken learned phrase never displaces a dictionary word the user did not pick; the manual custom dictionary's precedence is unchanged.
...
**Scope**: engine (PR1) + the four platform stores (iOS PR2, Android PR3, macOS + Windows PR4 — exact whole-buffer query; own-store follow-up 2026-09-21 PR-A iOS / PR-B Android / PR-C macOS + Windows, `docs/roadmap.md` § Own store). **No toggle** — learning is always on (USER 2026-09-20 「此功能不需要開關，預設都是開啟」; the `phraseLearningEnabled` setting shipped in #110–#112 was removed the same day). Manual rows keep 啟用自訂詞庫 as their own gate. `make build` before any mobile test.
...
- **The run in front of a segment is the boundary.** When the join is roman-ish (`continuous_word_space`), a boundary in front of a segment whose `raw_text` opens with a `-` run renders that run verbatim — in place of the word space and in place of the compound joiner alike (`tng--lai` → `tńg--lâi`, `tng-lai` → `tńg-lâi`, `tng---lai` → `tńg---lâi`). Nothing typed → the pre-existing policy (compound oracle `-`, else space). Under 無連字符 the run goes through `phonetics::api::hyphenless_display` (`--` → `·`, `-` → nothing, `----` → `··`), never a flat "two or more = one dot".
...
- **Unchanged**: `khiah` with nothing typed keeps every reading; TPS (§35 / §41 — the whole-buffer pin still reads the space barriers only, so `ㄒㄧ-` pins nothing) and the §34 literal; English; the 連字 / 輕聲 rendering rules (#129, §51 — the intra-word case since revised by §55).
...
- **Parity correction folded in (default ON)**: the continuous candidate path (`recase_all` → `raise_case`, raise-only, marker left as stored) never re-cased the marker, so under Caps Lock the strip read `SIAⁿ` beneath a `SIAᴺ` preedit, and a candidate pick committed `SIÂⁿ` where Enter committed `SIÂᴺ`. Step 5 now applies the same rule, so both read `SIAᴺ` — and under 羅馬字 / 漢羅濫 the literal and the dictionary row now read the same, so `adopt_collapsed_dict_identity` gives the literal the dictionary identity (one cell, one 詞頻 key) where Caps Lock used to leave two cells differing only in the marker's case.
...
Retro Codex review 2026-09-22 of PR #103, confirmed by engine probe: iOS `composingStart / Append / AppendHyphen / ReplaceLast / DeleteBackward / CommitDerived / EnterContinuous` and their Android twins sent the flag-free base `appConfig` (no `is_translate_swapped` / `output_both_scripts` / `hyphenless_roman`), a 2026-05-18 "commit entry points only" narrowing. Under Model B the engine re-renders the whole preedit from the request's config on every op (`api::combined_display(nailed, raw, config)` via `transition::step_response`), so after nailing 珠 under the default 漢字優先 the next keystroke read `珠 仔 guat` (word-boundary spaces) and 無連字符 showed `tâi-uân` again until the next commit. Desktop closed the same drift in #31 (S37); mobile never got it — invisible while 羅馬字優先 was the default, visible since #86 (2026-09-18).

- **The engine holds no settings of its own; every request carries the snapshot it should be rendered under** (`engine/dispatch/src/lib.rs` `request.config_snapshot`). No op is exempt: a nail and the keystroke after it must agree on the prefix. Only `Reset` / `SetSelectedCandidateIndex` / `QueryState` / `ResetContinuous`, which carry no config, stay outside.
...
- **Downstream passes see the typed form**: the POJ render keeps every `-` / space (`pàng-tāng-lāi` under POJ); 無連字符 drops the typed `-` and dots the `--` as it does any other hyphen (`pang-tang-lai` → `pàngtānglāi`, `pang--tang-lai` → `pàng·tānglāi`); the rendered dedupe runs whenever this pass changed a row.
