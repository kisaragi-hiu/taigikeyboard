...
- **Manual wins, in one place.** The platform's manual write primitive (iOS `CustomDictionaryRepository.upsert` / `batchImport`) takes over a learned row for the same `(roman, hanzi)` pair — the learned row and its side keys go, the pair is listed once as the user's — so the list editor, the CSV importer and the `.taigi` importer all obey the same rule; a learned row edited by the user becomes manual. A learn against an existing manual row is a no-op. Backup `.taigi` v3 carries `origin` + `learnCount`; an older file imports as manual.
...
**Scope**: engine (PR1) + the four platform stores (iOS PR2, Android PR3, macOS + Windows PR4 — provenance columns on the custom-dictionary table, exact whole-buffer query). **No toggle** — learning is always on (USER 2026-09-20 「此功能不需要開關，預設都是開啟」; the `phraseLearningEnabled` setting shipped in #110–#112 was removed the same day). Manual rows keep 啟用自訂詞庫 as their own gate. `make build` before any mobile test.
