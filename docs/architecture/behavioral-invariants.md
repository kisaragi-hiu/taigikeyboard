...
- **恢復預設設定 at the foot of the pane, same shape as 外觀 / 快捷鍵 / 辭典管理.** macOS `WideActionRow(.themeEditorResetAll)` → `SettingsStore.resetGeneralSettings()`; Windows `pages::reset_row` (the one gap-plus-card shape all four panes now share) → `ResetScope::General` → `SettingsDocument::reset_general()` over `keys::GENERAL_KEYS`. The roster is spelled out — `inputMode`, `toneInputScheme`, `isTranslateSwapped`, `displayLanguage`, `autoSpaceEnabled`, `candidateWindowEnabled`, `literalRomanCandidateEnabled` — and removed, not written. Not in it: the update bookkeeping and the remembered pane (not the user's settings), and every other pane's keys. `DisplayLanguageStore` observes its key, so the window re-reads in the shipped language on the same click.
...
- **The engine learns, the platform stores.** `commit_continuous`'s FINAL branch (pending consumed in full) emits `Effect.PhraseLearned { hanji, canonical_tl, syllable_count }` (oneof kind 11, after `NextWordWordSelected`) when the composition has ≥ 2 nailed segments, every segment carries `NailedSegment.hanji` and a non-empty `association_tl`, and the summed `syllable_count` ≤ `MAX_LEARNED_PHRASE_SYLLABLES` (6). `hanji` = the segments' hanji concatenated; `canonical_tl` = the segments' canonical TL joined with `-`, a piece already opening with the khinsiann `--` keeps it (`kì` + `--khí-lâi` → `kì--khí-lâi`). A mid-commit, a single segment, a hanji-less segment (§34 literal, OOV synth, roman-only custom row), an empty TL, an abort or a backspace-unnailed segment never learn (`transition::learned_phrase`).
...
**Scope**: engine (PR1) + the four platform stores (iOS PR2, Android PR3, macOS + Windows PR4 — provenance columns on the custom-dictionary table, exact whole-buffer query, `phraseLearningEnabled` default ON). `make build` before any mobile test.

**Tests**: `engine/composing/tests/continuous_learned_phrase.rs` (learning: join, khinsiann, three singles, single segment, hanji-less, empty TL, 6 vs 7 syllables; recall: no-dict lead, khinsiann key, POJ-typed buffer, dictionary homophone beats learned until picked, dictionary duplicate listed once, custom outranks learned, empty rows ignored). Offline repro: `DUMP_LEARNED="記起來:kì--khí-lâi" DUMP_BITMASK=8159 DUMP_INPUTS=kikhilai cargo test -p composing --test candidate_dump -- --ignored --nocapture`.
