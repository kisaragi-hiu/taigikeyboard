...
6. **`KeyKey/YahooKeyKey-Source-1.1.2528/Distributions/Takao/`** (#27) — the original 2012 shipping pipeline behind #26's lexicon: `DatabaseCooker/` (`Makefile.SmartMandarin`, `Schema.sql`, `AssociatedPhraseCooker.rb`, `generate-noise.rb`) cooks `Manjusri/Tools/*.rb` corpus output (`extract-corpus.rb` → `lm-bpmfmarker.rb` → `prepare-sql.rb`) into the SQLite LM; per-platform installer folders (`Installer-OSX-DB` / `-IMK` / `-TSM` / `Installer-Windows`), `OnlineData` + `OnlineDataTemplates` for the hot-keyword feed, `Keyring` for signed module packages. Read for the shape of a corpus → LM → installer build graph; the corpus itself is not in the repo.
...
### 27. Yahoo! KeyKey 1.1.2528 source drop — `references/KeyKey/`

- **What**: the 2012 open-sourcing of Yahoo! 奇摩輸入法 (KeyKey), the OpenVanilla-based Bopomofo IME that #26 ChiaKey revives. Two top-level trees: `YahooKeyKey-Source-1.1.2528/` (~120k LOC across C++ frameworks, Obj-C++ macOS loaders, C++-CLI Windows loader, 24 OpenVanilla module packages, Ruby/Perl utilities) and `DataSource-1.1.2528/` (README pointers only — the SinicaCorpus / BPMFMappings / YahooSearchTerms data is **not** included). Frozen: `81e05f0` (2012-12-04), no upstream activity; org fork `taigikeyboard/KeyKey`.
- **Why we care**: two things #26 no longer carries. (1) **The Windows loader** — `Loaders/Windows-IMM/` is a thin IMM32 `BaseIME` DLL per host process talking MS-RPC to one `BaseIMEServer.exe` that owns the engine, every database and every UI window; the IDL is the cleanest statement we have of "what a keystroke round trip must return" for an out-of-process desktop IME. (2) **The distribution pipeline** — `Distributions/Takao/` + `Frameworks/Manjusri/Tools/` show the full corpus → LM → SQLite → per-platform installer graph that #26 inherits but has since trimmed. Everything else (Manjusri walker, `Formosa` Bopomofo, `PlainVanilla` loader, learning) is the ancestor of #26 and better read there.
- **Where to look**:
  - `Loaders/Windows-IMM/BaseIMERPC/BIServerRPCInterface.idl` — `BISHandleKey` (one call → committed / composition / reading / candidate page / `actionVerb`), `BISActivate` / `BISDeactivate` residue hand-back, DB-update restart verbs, phrase-DB CRUD as string-dispatched function names
  - `Loaders/Windows-IMM/BaseIME/` — `IMEClient.cpp` (RPC client + reconnect), `BaseIME.cpp` (the IMM32 export surface, listed at the top of `BaseIME.h`), `IMEInputBuffer` / `IMECandidatePanel` / `IMEStatusBar` (client-side fallback UI)
  - `Loaders/Windows-IMM/BaseIMEServer/` — `IMEServer` singleton, `RPCService.cpp`, `YKSignedModuleLoadingSystem.h` (signed module packages), `RemoteDataFetcher.h` (online hot-keyword / canned-message feeds)
  - `Loaders/Windows-IMM/ServerShooter/ServerShooter.cpp` — kill server, swap `Supplement.db`, relaunch from `HKLM` `ServerLocation`
  - `Studies/Win32RPCStudy/` — the minimal RPC client/server spike the loader grew from
  - `Distributions/Takao/README.markdown` + `DatabaseCooker/` + `Frameworks/Manjusri/Tools/*.rb` — build graph
  - `ModulePackages/` — `OVAFHomophoneLookup`, `OVAFReverseLookup`, `OVOFHanConvert` (簡繁), `YKAFOneKey`, `YKAFPhraseAware`, `OVAFBopomofoCorrection` — the "around-filter" / "output-filter" plugin taxonomy
  - `Loaders/OSX-IMK/CV*.mm` — 2012 Cocoa candidate / symbol / smiley / notify windows (superseded by #26 and our `macos/`)
- **Inspiration takeaways**: (1) `BISHandleKey`'s one-round-trip result shape (commit + composition + reading + candidate page + cursor + verb) as a checklist for our TSF `Runtime` response; (2) `actionVerb` / `actionParameter` as an escape hatch for "engine asks host to do X" (open preferences, send fake key) instead of growing the RPC surface; (3) around-filter / output-filter plugin split (`OVAF*` / `OVOF*`) as a naming model for post-processing stages (簡繁, full-width, anti-注音文); (4) Takao `DatabaseCooker` as the reference shape for a corpus → LM → installer Makefile graph.
- **Deliberately not adopted**: IMM32 (dead API; we are TSF), MS-RPC out-of-process server (our engine is in-process Rust, see #18/#19 for the modern variants of this argument), C++-CLI preference UI, OpenVanilla module loader, `sqlite-cerod-see` encrypted read-only DB (commercial), online hot-keyword feed, Bopomofo reading buffer. Mandarin phonetic tables do not transfer (Core Principle #3).
